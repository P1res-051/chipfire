...
import { ConversationList } from '../components/UserConversationList'
import { ConversationDetail } from '../components/UserConversationDetail'
import { UserConversationFilters } from '../components/UserConversationFilters'
...
export function UserInboxPage() {
  ...
  // Buscar conversas (user vê apenas suas instâncias)
  const conversationsQuery = useQuery({
    queryKey: ['user', 'conversations', filters],
    queryFn: async () => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[350px_1fr] gap-4">
      ...
      <div className="space-y-4">
        <div>
          <h1 className="text-2xl font-semibold mb-1">Minhas Conversas</h1>
      ...
      <div>
        {selectedConversation ? (
          <ConversationDetail
            ...
            onOptOut={() => {
              qc.invalidateQueries({ queryKey: ['user', 'conversations'] })
